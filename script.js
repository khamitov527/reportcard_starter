...
const studentInformation = {
  name: "Akylbek Khamitov",
  grade: "3.8",
  advisor: "John Doe",
  major: "Computer Science",
  graduationYear: "2024",
  imageUrl: "https://media.wired.com/photos/5cdefb92b86e041493d389df/1:1/w_988,h_988,c_limit/Culture-Grumpy-Cat-487386121.jpg",
}
...
/**
 * Modify the img el using `setAttribute` so that that the src attribute
...
function addUpStudentCredits(reportCardTableElement) {
  ...
  const totalCredits = reportCardTableElement.querySelector("#total-credits")

  let total = 0;

  allCredits.forEach((credit) => {
    const creditAmount = Number(credit.innerHTML)
    total += creditAmount
  })

  totalCredits.innerHTML = total + " credits"
}
...
function calculateSemesterGpa(reportCardTableElement) {
  // code goes here
  const allGpa = reportCardTableElement.querySelectorAll(".gpa")
  let totalCourses = 0
  let totalPoints = 0
  ...
  const totalPointsElement = reportCardTableElement.querySelector("#total-pts")
  if (totalPointsElement) totalPointsElement.innerHTML = totalPoints
  const cumulativeGpaElement = reportCardTableElement.querySelector("#gpa")
  if (cumulativeGpaElement) cumulativeGpaElement.innerHTML = Number(totalPoints / totalCourses).toFixed(2)
}
